...
updated_at: 2026-05-09T13:49:41Z
...
Created internal/ui/tasklist/update.go with Update() method handling key events (j/k/g/G/H navigation, skipping section separators), filter messages (ProjectSelectedMsg, LabelSelectedMsg, FilterSelectedMsg), SyncCompleteMsg (reload preserving selection), ToggleCompletedMsg (toggle and reload), and WindowSizeMsg. Includes keyMatches helper, navigate/goToBoundary helpers, emitTaskSelected command, firstTaskID and taskIDExists helpers. Removed Update stub from model.go. go build and go vet pass.
